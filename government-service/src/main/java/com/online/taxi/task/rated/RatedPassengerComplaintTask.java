...
public class RatedPassengerComplaintTask extends AbstractSupervisionTask {
    ...
    private boolean execute(Integer id) {
        ...
        return tryComposeData(maxTimes, p -> {
            RatedPassengerComplaintDto data = null;
            ipcType = OTIpcDef.IpcType.ratedPassengerComplaint;
            try {
                data = ratedMapper.selectRatedPassengerComplaint(id);
                messageMap.put("OrderId", data.getOrderNumber());
                messageMap.put("ComplaintTime", formatDateTime(data.getUpdateTime(), DateTimePatternEnum.DateTime));
                messageMap.put("Detail", data.getContent());
                return true;
            } catch (Exception e) {
